<template>
  <section class="bg-white"
    :class="{ 'fixed inset-0 z-50 fullscreen-map-active': isFullscreen, 'py-12 md:py-24': !isFullscreen }">
    <div class="h-full" :class="{ 'container mx-auto px-4 md:px-6': !isFullscreen }">
      <div v-if="!isFullscreen" class="text-center mb-6 md:mb-12" data-aos="fade-up">
        <h2 class="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-parque-dark mb-4 md:mb-8">
          Barrio privado <span class="text-parque-lime">Parque Pinares</span>
        </h2>
      </div>

      <!-- Map Interactive -->
      <div class="w-full h-full" :class="{ 'mx-auto': !isFullscreen }">
        <div class="bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col"
          :class="[isFullscreen ? 'h-screen' : 'h-full', { 'rounded-none': isFullscreen }]" data-aos="fade-up"
          data-aos-delay="50">
          ...
          <div class="relative flex-1">
            <div class="map-container overflow-hidden bg-gray-100 w-full"
              :class="isFullscreen ? 'h-full' : 'h-56 sm:h-64 md:h-80 lg:h-96'" ref="mapContainer">
              <img v-if="mapImageUrl" ref="mapImage" :key="mapImageUrl" :src="mapImageUrl"
                alt="Plan maestro interactivo del proyecto Parque Pinares"
                class="select-none cursor-grab active:cursor-grabbing" draggable="false">
            </div>
            ...
            <div class="absolute bottom-4 md:bottom-6 right-4 md:right-6 flex flex-col gap-2 z-10">
              <button @click="zoomIn"
                ...
                aria-label="Aumentar zoom" title="Aumentar zoom (+)">
                <span class="text-lg md:text-2xl font-bold">+</span>
              </button>

              <button @click="zoomOut"
                class="bg-parque-lime text-white w-10 md:w-12 h-10 md:h-12 rounded-full shadow-lg hover:shadow-xl hover:scale-110 transition-all duration-200 flex items-center justify-center"
                aria-label="Reducir zoom" title="Reducir zoom (-)">
              ...
              <button @click="resetMap"
                ...
                aria-label="Reiniciar vista" title="Reiniciar vista">
                <svg class="w-5 md:w-6 h-5 md:h-6" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M4 12a8 8 0 018-8V2.5M4 12a8 8 0 008 8v1.5M20 12a8 8 0 01-8 8M20 12a8 8 0 00-8-8"
            ...
            <div
              class="absolute top-4 md:top-6 left-4 md:left-6 bg-white/90 backdrop-blur px-3 md:px-4 py-1 md:py-2 rounded-full text-xs md:text-sm font-semibold text-parque-dark shadow-lg">
          ...
          <div v-if="isFullscreen"
            class="absolute bottom-6 left-6 bg-white/95 backdrop-blur px-6 py-5 rounded-lg shadow-xl z-20">
